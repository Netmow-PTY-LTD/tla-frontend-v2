...
export async function middleware(request) {
  ...
  if (publicAuthPages.includes(pathname)) {
    if (token) {
      ...
      if (user) {
        ...
        const redirectPath = dashboardRoutes[user.regUserType] || '/';
  ...
  // PROTECTED ROUTES
  const protectedRoutes = ['/admin', '/client', '/lawyer', '/marketing'];
  ...
  const routeAccess = {
    admin: ['/admin', '/client', '/lawyer', '/marketing'],
    lawyer: ['/lawyer'],
    client: ['/client'],
    marketer: ['/marketing'],
  };
  ...
  if (!isAllowed) {
    ...
    if (role === 'marketer') redirectPath = '/marketing';
