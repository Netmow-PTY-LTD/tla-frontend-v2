...
export default function ClientLeadRegistrationModal({
  ...
  const [clientRegistrationDraft, { isLoading: isDraftLoading }] =
    useClientRegistrationDraftMutation();

  const isSubmitting = isDraftLoading || isSubmittingLegacy;
  ...
  const handleNext = async () => {
    ...
    // const transformedQuestions = questionsPayload.map((q) => ({
    //   ...q,
    //   checkedOptionsDetails: q.checkedOptionsDetails.map((opt) => ({
    //     optionId: opt.id,
    //     label: opt.name,
    //     extraData: opt.idExtraData,
    //   })),
    // }));
    ...
    try {
      ...
      console.log('client registration draft res', res);
      if (res?.success && res?.data?.clientDraftId) {
        // setDraftId(res.data.clientDraftId);
        // setShowOtp(true);
        // showSuccessToast('Draft saved. Please verify your email.');
        setModalOpen(false);
        ...
        <div className="max-h-[90vh] overflow-auto">
          {isQuestionsLoading || !selectedServiceWiseQuestions?.length ? (
            ...
            <div className="space-y-6 pt-6 px-6">
              ...
              <p className="text-blue-500 text-sm">
                <b>Please note:</b> A verification link will be sent to your email address.
          ...
          {isQuestionsLoading ||
            (selectedServiceWiseQuestions?.length > 0 && (
              <div
                className={`flex ${step === 0 ? 'justify-end' : 'justify-between'
                  } mt-8 px-6`}
              >
                {step !== 0 && (
                  <Button
          ...
          <div className="flex flex-col items-center text-center space-y-5">
            ...
            <div className="bg-[var(--color-special)] bg-opacity-5 p-6 rounded-xl border border-[var(--color-special)] border-opacity-10 w-full text-center">
              <p className="text-white font-semibold flex items-center justify-center gap-2 mb-2 uppercase text-xs tracking-wider">
                <Mail className="w-4 h-4 text-white" /> Email Verification Required
              </p>
