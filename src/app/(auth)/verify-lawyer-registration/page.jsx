...
export default function VerifyClientRegistrationPage() {
    ...
    const [verifyEmail] = useLawyerRegistrationVerifyEmailMutation();
    const [commitRegistration] = useLawyerRegistrationCommitMutation();
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            if (verifyRes?.success) {
                ...
                console.log('commitRes for Lawyer registration', commitRes);
                ...
                if (commitRes?.success && commitRes?.token) {
                    ...
                    if (userPayload) {
                        // Login the user
                        dispatch(setUser({ user: { ...commitRes?.data, country: userPayload?.country }, token }));
                        setStatus('success');
                        // Optional: Redirect after a delay
                        setTimeout(() => {
                            const userType = commitRes?.data?.regUserType;
                            if (userType === 'lawyer') {
