...
export default function SendSmsModalClient({ openSms, setOpenSms, info }) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!toPhone || !leadId || !responseId) {
        toast.error('Missing case/contact information');
        return;
      }
      ...
      toast.error(
        error?.data?.message || 'An error occurred while sending SMS'
      );
    }
  };
  ...
  return (
    <div>
      <Modal open={openSms} onOpenChange={setOpenSms}>
        <FormWrapper onSubmit={onSubmit}>
          <div className="text-center max-w-[90%] mx-auto">
            ...
            <p className="text-gray-600">
              We'll send your SMS to {lead?.name} and send you any replies from
              them over email.
            </p>
          </div>
