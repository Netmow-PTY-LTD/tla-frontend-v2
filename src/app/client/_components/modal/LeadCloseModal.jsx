...
export default function LeadCloseModal({
  ...
  const handleSave = async () => {
    if (!reason.trim()) {
      toast.error("Please provide a reason before closing the case.");
      return;
    }
    ...
    try {
      ...
      toast.success(result?.data?.message || "Case closed successfully!");
      setReason("");
      onOpenChange(false);
    } catch (error) {
      toast.error(error?.data?.message || "Failed to close the case");
    }
  };
  ...
  return (
    <Modal
      title="Close Case"
      description="Provide a reason for closing this case"
      open={open}
    ...
    >
      ...
      <div className="space-y-6 p-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-500">Provide a reason for closing this case.</p>
        </div>
