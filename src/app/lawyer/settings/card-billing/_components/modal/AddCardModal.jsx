...
const AddCardForm = ({ setOpen, onCardAdded }) => {
  ...
  const handleAddCard = async (e) => {
    ...
    try {
      ...
      if (stripeError) {
        ...
        showErrorToast(stripeError.message);
      } else {
        setError(null);
          ...
          options={{
            ...
            fields: {
              billingDetails: {
                name: 'auto',
                email: 'auto',
                address: 'auto', // Stripe handles country/region automatically
              },
            },
          }}
        />
  ...
  useEffect(() => {
    if (open && !clientSecret) {
      setupPaymentIntent()
        ...
        .catch((err) => {
          console.error(err);
          showErrorToast('Failed to initialize payment element');
        });
    }
    if (!open) {
      setClientSecret(null);
    }
  }, [open, clientSecret, setupPaymentIntent]);
